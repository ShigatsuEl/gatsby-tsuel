...
**만약 서버를 재시작하는데 오류가 발생한다면 지난 글에서 작성한 `gatsby-node.js`를 제거하지 않았으며** 

**`gatsby-config.js`에서 실수를 해서 markdown 파일과 겹쳐 오류가 발생한 것일 수 있습니다.**

실수한 부분이 없는지 확인하시고 [Gatsby Plugin Mdx](https://www.gatsbyjs.com/plugins/gatsby-plugin-mdx/) 또는 [Gatsby Mdx](https://www.gatsbyjs.com/docs/how-to/routing/mdx/)를 참고 하십시오.
...
정확하지는 않지만 우리가 만든 `test-mdx` 라우트와 Link의 path가 일치하지 않아 포스트를 클릭할 시 `Not Found Page`로 이동할 수 있습니다.
...
위는 제가 경로를 해결하기 위해 사용한 방법입니다. :cry:
