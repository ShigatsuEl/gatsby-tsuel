...
- Login Router
  ...
  - Game
    ...
    게임이 시작되면 인원수에 맞게 랜덤으로 마피아와 시민을 나누기 -> User 모델(+role)

    - 밤

      마피아만 채팅 활성화 & 채팅 볼 수 있음(15초) -> Chat 모델(userName: [User 모델], content, visible)

      마피아 투표 -> Game 모델(+aliveUserList: [User 모델]) / role을 통해 살아남은 시민과 마피아를 aliveUserList에 반영

      클라이언트 측에서 soket.io 필요(단 한명만 지목 가능하게 함)
    ...
    - 낮
      ...
      낮 종료 시, 가장 많은 투표를 받은 용의자 선정(role 업데이트)
